...
inline Array<T>::Array(int length, int start_index){
	try {
		if (length < 0) {
			Exception negLength;
			std::string msg = "Error: <0 Sized Array";
			negLength.setMessage(msg);
			
				throw negLength;
		}
		 T* m_array = new T[length];
		int m_size = length;
		int m_start_index = start_index;
	}
...
inline void Array<T>::setLength(int other)
{
	try
	{
		if (other < 0) {
			Exception negLength;
			int sizeMsg = strlen("Error: Neg Sized Array not possible") + 1;
	...
	try
	{
		if (other == 0) {
			Exception negLength;
			int sizeMsg = strlen("Error: Zero Sized Array not possible") + 1;
			char* msg = new char[sizeMsg];
